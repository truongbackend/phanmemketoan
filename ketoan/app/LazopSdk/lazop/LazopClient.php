...
class LazopClient
{
	...
	protected function logApiError($requestUrl, $errorCode, $responseTxt)
	{
        ...
        // Lấy giá trị LAZOP_SDK_WORK_DIR từ global scope nếu có, nếu không thì lấy dirname(__DIR__, 2)
        $workDir = (\defined('LAZOP_SDK_WORK_DIR')) ? \constant('LAZOP_SDK_WORK_DIR') : \dirname(__DIR__, 2);
		$logger->conf["log_file"] = rtrim($workDir, '\\/') . '/' . "logs/lazopsdk.log." . date("Y-m-d");
